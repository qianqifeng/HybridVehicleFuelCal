%{
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 % @file               VehicleAccModel.m
 % @author             New Energy Automobile Team
 % @version            V1.0  
 % @data               22-April-2019 
 % @brief              Calculate the vehicle accelerate dynamics
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 % @attention
 %
...
%}
function [whPow,whTt,whRotaSpd,FoutFa] = VehicleAccModel(vehPara,cycPara,aerodyPara,wheelPara)
    %vehicle parameter
...
%     cycAngle = atan(cycGlope/100);                          % Slope converted to angle
    cycAngle = 0*ones(length(cycTime),1);                                           % Slope Mandatory equal to zero
    cycVe = cycPara.ve;
    ...
    FoutFa = Fr+Fa+Fi;
    figure;
    plot(cycTime,cycV*3.6,'r--','LineWidth',2);xlabel('accTIme/s');ylabel('accVe/km/h');
    figure;
    hold on;
    ...
    legend('Fr','Fi','Fa','Fj','Ft');xlabel('time/s');ylabel('F/N');
    hold off;
